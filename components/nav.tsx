...
export function Nav() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-ink/5 bg-porcelain/78 backdrop-blur-xl">
      <nav className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 md:px-10">
        ...
        <div className="flex items-center gap-3">
          ...
          <button type="button" aria-label={isOpen ? "Close menu" : "Open menu"} aria-expanded={isOpen} onClick={() => setIsOpen((current) => !current)} className="grid h-10 w-10 place-items-center rounded-full border border-ink/10 text-ink transition hover:border-ink/25 hover:bg-white">
            {isOpen ? <X className="h-4 w-4" aria-hidden="true" /> : <Menu className="h-4 w-4" aria-hidden="true" />}
          </button>
        </div>
      </nav>
      {isOpen ? (
        <div className="border-t border-ink/5 bg-porcelain/92">
